...
- `check_ins.insert` — persist one row per chunk check-in (5 per completed session).
- `check_in_turns.insert` — persist every turn in a check-in's conversation (typically 7–10 rows per check-in; agent + patient rows interleaved).
