...
INPUT:   Patient opens WAVE (via proactive notification, lock-screen
         ...
         physical / unknown / other). The "other" trigger reveals a
         short free-text field so the patient can name their own
...
PROCESS: Once the intake safety screen clears, WAVE runs a 12–15 minute
         ...
         check-in: `lora-check-in-1` through `lora-check-in-5` plus
         `lora-reflection` at close. The 5 narrated chunks themselves
         are delivered from a fixed scripted bank (MVP: on-screen text
         + wave animation; V2: Kokoro TTS audio). Crisis triage runs
         on base Gemma with no LoRA; routing to 988 / SAMHSA and the
         intake safety-screen handoff are both rule-based. No cloud
         LLM is ever called. Medication status captured at intake is
         passed as context into every check-in prompt so Gemma can
         reference it in validation copy (e.g. "your Suboxone is
         already dampening this — what you feel at 7 would be a 9
         without it"). Per-model reference: `docs/models.md`. Training
         process: `docs/model-training.md`.
...
2. **Intake**: Patient taps intensity (e.g. 7/10), medication status (e.g. "took Suboxone on time"), and trigger. Trigger options are `social | stress | physical | unknown | other`; selecting **other** reveals a single-line free-text field capped at 80 characters so the patient can name their own trigger. No typing is required for any of the non-`other` paths. ~30 seconds.
...
4. **Chunk 1 — Intro + settling + urge awareness (~2 min)**: Scripted narration welcomes the patient, settles them in, and introduces the wave metaphor. Pauses are timed to the instruction (see Session Runtime Requirements). The continuous ambient-wave audio layer starts here and plays uninterrupted through every chunk and check-in until the reflection screen is dismissed.
...
7. **Chunk 3 — Sound / visualization anchor (~2–3 min)** → **Check-in 3 (multi-turn)**: Turn 2 opener is "Could you hold onto the sound of water, or was it hard to stay with?" Turn 3 branches on whether the anchor worked; if it didn't, Gemma validates and offers one technique (real-sound anchoring, or thought labeling, or normalizing urge intensification). Turn 5 closes.
8. **Chunk 4 — Breathing exercise, 4-4-6 (~3–4 min)** → **Check-in 4 (multi-turn)**: Inhale 4s / hold 4s / exhale 6s, guided first then unguided. The wave animation rises on inhale, holds at peak, recedes on exhale, at exactly the counted pace. Turn 2 opener is "How did the breathing feel — were you able to follow your own count?" Turn 3 branches on tight chest / intruding thoughts / breath-induced anxiety with the obstacle-library technique. Turn 5 closes.
...
10. **Reflection + next step**: Post-session screen: "You surfed a 7 down to 2. That's your 12th session. On medication days you drop 5.1 points on average." Optional one-line journal. If `usedSubstanceToday: true` was logged at the intake safety screen, the reflection LoRA may acknowledge (in a trauma-informed way) that the patient chose to surf a craving even after using. Patient picks a 10-minute next action (call someone / walk / water / hands / rest). Session logs and closes; the ambient-wave audio layer fades out.
...
**Clinical rationale:** Urges peak within 15–20 minutes and subside within 30 minutes without being fed. A 12–15 minute structured session catches the full arc of an urge. Five check-ins provide enough data points to adapt without fragmenting the meditative experience. Chunk scripts live in `client/lib/prompts/session-script.ts` and must not be regenerated at runtime; they are clinically reviewed copy.
...
1. **Pause duration is derived from the spoken instruction.** If the narration says "count to 6", the pause is exactly 6 seconds. If it says "count to 4", it is 4 seconds. If the script marks `[20-second pause]`, it is 20 seconds. Inhale / hold / exhale segments use the counted value as their duration (4 / 4 / 6). The segment data structure stores `duration` as the single source of truth and the rendered instruction text must match it — a linter test asserts the two agree.
...
| 2 (Body awareness, after Chunk 2) | "Craving score right now, 1 to 10?" | "[score reflection.] Were you able to locate where the urge lives in your body?" |
| 3 (Anchor, after Chunk 3) | "How intense is the craving now, 1 to 10?" | "[score reflection.] Could you hold onto the sound of water, or was it hard to stay with?" |
| 4 (Breathing, after Chunk 4) | "Craving score, 1 to 10?" | "[score reflection.] How did the breathing feel — were you able to follow your own count, or did something get in the way?" |
...
`[score reflection]` is filled in at runtime from the score-tracking response table below. These openers live as typed data in `client/lib/prompts/check-in-openers.ts`; they are scripted fallbacks, not LLM output. The LoRA-generated copy goes in Turns 3 and 4.
...
interface SessionState {
  ...
  userProfile: {
    ...
    triggerOther: string | null   // only set when trigger === 'other'
...
**No permanent backend** — the settled product architecture runs **Gemma 4 E2B-it end-to-end in the browser** via `@huggingface/transformers` + WebGPU for the web demo, and the same model on-device via LiteRT inside a future React Native app.

The current checked-in `client/` demo has temporary Next.js Route Handlers that call OpenAI `gpt-5-mini` while the in-browser Gemma + LoRA stack is unfinished. They are scaffolding behind the same `client/lib/gemma/*` boundaries the final runtime will keep, not a product backend.
...
Temporary routes in the current web demo:

- `POST /api/checkin` — streams the next multi-turn check-in reply via `gpt-5-mini`; called by `client/lib/gemma/checkin.ts`.
- `POST /api/narrate/reflection` — streams reflection progress titles and a validated reflection payload; called by `generateReflection()` in `client/lib/gemma/session.ts`.
- `POST /api/insights` — regenerates insight cards from mock session history; called by `/insights`.
- `POST /api/narrate`, `POST /api/narrate/stream`, and `POST /api/chunk` — legacy or deprecated scaffolding. `generateChunk()` currently returns scripted chunks from the fallback bank and does not call `/api/chunk`.

Final runtime: every check-in turn is generated by Gemma 4 E2B running inside the browser tab — base model + the appropriate check-in LoRA (see `docs/models.md`). The 5 chunk scripts are scripted, clinician-reviewed copy and are not generated at runtime. The per-check-in Turn 1 and Turn 2 openers are scripted fallbacks in `client/lib/prompts/check-in-openers.ts`; the LoRA generates the adaptive later turns.
...
- **Offline-first (everywhere)** — the final session path makes zero LLM network requests on mobile **and** the web demo. Mobile runs Gemma 4 E2B via LiteRT; the web demo runs Gemma 4 E2B via `@huggingface/transformers` + WebGPU. The current `gpt-5-mini` route handlers are temporary scaffolding only. A single scripted narration bank under `client/lib/prompts/` is the fallback when WebGPU is unavailable, a temporary model route fails twice, or Gemma output fails Zod validation twice.
- **Pause-duration invariant** — every `pause` or `breath` segment's `duration` field must equal the count spoken in the nearest preceding `text` / `instruction`. A unit test over every chunk asserts this. "Count to 6" and a 4-second pause is a ship-blocker.
...
- [ ] Three-tap intake: no typing required to start a session on any path **except** the `other` trigger, which reveals a single free-text field.
...
- [ ] Pause-duration invariant test passes: every `pause` / `breath` segment's `duration` equals the count spoken in the paired text.
...
3. **WebGPU unavailable at demo time** — the web demo relies on in-browser Gemma 4 via WebGPU; Safari and older machines may not support it. Mitigation: the landing page detects WebGPU on load, pre-warms the model download, and falls through to the scripted local narration bank if the runtime is unavailable or a model output fails Zod validation twice. The scripted bank covers every `(matType, medicationStatus, chunkNumber)` cell and is exercised in every PR's manual test.
...
5. **Pause/instruction drift** — a copy edit changes "count to 6" to "count to 5" without updating the paired `duration`, and the wave animation or spoken count no longer matches the pause. Mitigation: the pause-duration invariant test in Success Criteria; chunk scripts live in a single typed file with inline `duration` fields so a reviewer can see the mismatch in the diff.
