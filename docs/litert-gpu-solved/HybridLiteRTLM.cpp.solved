...
// Wave#17 Phase 1 diagnostics: os_log reaches idevicesyslog (stderr does
// not), and we redirect LiteRT/absl stderr to a file in the model dir so
// its GPU-init failure reason can be pulled off-device.
#include <os/log.h>
#include <cstdio>
...
// Wave#17 Phase 2: included LAST (after NitroModules/ReactCommon headers) —
// CoreFoundation's macros break the RN bridging template parsing if pulled
// in earlier. Used to preload the v0.11.0 GPU/Metal accelerator plugins by
// absolute path from the app bundle so the runtime's leaf-name dlopen
// resolves the already-loaded image (dyld dedups by install name).
#include <dlfcn.h>
#ifdef __APPLE__
...
void HybridLiteRTLM::createNewConversation() {
  ...
  // Create conversation config (v0.11.0: create() takes no args; config is
  // applied via setters — Wave#13 bridge port).
  conv_config_ = litert_lm_conversation_config_create();
  if (!conv_config_) {
    throw std::runtime_error("Failed to create conversation config");
  }
  if (session_config_) {
    litert_lm_conversation_config_set_session_config(conv_config_,
                                                     session_config_);
  }
  if (systemMsgPtr) {
    litert_lm_conversation_config_set_system_message(conv_config_,
                                                     systemMsgPtr);
  }
  litert_lm_conversation_config_set_enable_constrained_decoding(conv_config_,
                                                                false);
...
void HybridLiteRTLM::loadModelInternal(
  ...
  // Wave#17 Phase 1: temporarily 0 to surface the Metal/MLDrift GPU
  // engine-creation failure that triggers the silent CPU fallback.
  ...
  auto tryCreateEngine = [&](const char* backend, const char* visionBackend) -> bool {
    ...
    // Wave#13 lever #1: disable v0.11.0 MTP/speculative decoding — the
    // tf_lite_mtp_drafter subgraph's DIV/SOFTMAX ops fail XNNPACK prepare
    // on the from-source build. See if skipping the drafter unblocks
    // engine creation (Metal already takes the main transformer 100%).
    litert_lm_engine_settings_set_enable_speculative_decoding(settings, false);
    os_log(OS_LOG_DEFAULT, "WAVE17 speculative_decoding=false set");
    ...
    // Wave#17 Phase 1: one-time redirect of LiteRT/absl stderr to a file
    // in the model dir so its GPU-init failure reason can be pulled
    // off-device (idevicesyslog does not capture iOS app stderr).
    static bool stderrRedirected = false;
    if (!stderrRedirected) {
      std::string logPath = cacheDir + "/litert-stderr.log";
      freopen(logPath.c_str(), "w", stderr);
      stderrRedirected = true;
      os_log(OS_LOG_DEFAULT, "WAVE17 stderr -> %{public}s", logPath.c_str());
    }

    os_log(OS_LOG_DEFAULT,
           "WAVE17 tryCreateEngine backend=%{public}s vision=%{public}s",
           backend ? backend : "(null)",
           visionBackend ? visionBackend : "(null)");
    ...
    os_log(OS_LOG_DEFAULT,
           "WAVE17 tryCreateEngine backend=%{public}s result=%{public}s",
           backend ? backend : "(null)",
           engine_ != nullptr ? "OK" : "FAILED(null)");

    return engine_ != nullptr;
  };
  
  // Wave#17 Phase 2: preload the v0.11.0 GPU/Metal accelerator plugins by
  // absolute path from the app bundle's Frameworks dir. The runtime later
  // dlopens them by leaf name; preloading the same images first makes that
  // resolve (dyld returns the already-loaded library).
  static bool acceleratorsPreloaded = false;
  if (!acceleratorsPreloaded) {
    acceleratorsPreloaded = true;
    ...
    if (mainBundle) {
      ...
      if (url) {
        gotPath = CFURLGetFileSystemRepresentation(
            url, true, reinterpret_cast<UInt8*>(bundlePath), PATH_MAX);
        CFRelease(url);
      }
    }
    // Wave#13/#18 CLiteRTLM (PhoneClaw) architecture: the two GPU plugins are
    // dlopen'd at runtime. They ship as .framework binaries but keep dylib
    // install-names (@rpath/libLiteRt*Metal*.dylib), so preloading the
    // framework binary by absolute path makes LiteRT's later leaf-name
    // dlopen("libLiteRtMetalAccelerator.dylib") resolve the already-loaded
    // image. Engine (LiteRTLM) + GemmaModelConstraintProvider are
    // LC_LOAD_DYLIB-linked and loaded at launch via @rpath — no preload.
    const char* plugins[] = {
      "LiteRtMetalAccelerator.framework/LiteRtMetalAccelerator",
      "LiteRtTopKMetalSampler.framework/LiteRtTopKMetalSampler",
    };
    for (const char* rel : plugins) {
      std::string p = std::string(bundlePath) + "/Frameworks/" + rel;
      void* h = gotPath ? dlopen(p.c_str(), RTLD_NOW | RTLD_GLOBAL) : nullptr;
      os_log(OS_LOG_DEFAULT,
             "WAVE17 preload %{public}s -> %{public}s",
             rel, h ? "OK" : (dlerror() ? dlerror() : "FAILED"));
    }
  }
  ...
  if (!tryCreateEngine(primaryBackend, primaryBackend)) {
    ...
    os_log(OS_LOG_DEFAULT,
           "WAVE17 primary=%{public}s FAILED -> resolved backend=%{public}s",
           primaryBackend,
           backend_ == Backend::CPU ? "cpu" : "gpu/other");
  } else {
    os_log(OS_LOG_DEFAULT,
           "WAVE17 primary backend=%{public}s created OK (no fallback)",
           primaryBackend);
  }
  ...
  if (session_config_) {
    ...
    sampler.type = kLiteRtLmSamplerTypeTopP;  // Wave#13: v0.11.0 enum prefix
