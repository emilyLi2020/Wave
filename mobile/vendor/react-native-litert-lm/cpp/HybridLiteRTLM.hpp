...
private:
  ...
  double topK_ = 40.0;
  double topP_ = 0.95;
  double maxTokens_ = 1024.0;  // legacy/back-compat input (sets both below)
  // Unconflated token budgets (see Wave#14 / upstream hung-yueh#10):
  // engine cache size vs per-call decode cap are distinct constraints.
  size_t engineMaxTokens_ = 2048;  // -> litert_lm_engine_settings_set_max_num_tokens
  size_t outputMaxTokens_ = 1024;  // -> litert_lm_session_config_set_max_output_tokens
