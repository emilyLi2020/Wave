...
- **WAVE web app** lives under `client/` (Next.js 16 + TypeScript + Tailwind v4).
...
Gemma 4 narration is swappable via `NARRATION_PROVIDER` in `client/.env.local`:
...
The Route Handler at `client/app/api/session/narrate` switches between them. `webllm` bypasses it entirely.
